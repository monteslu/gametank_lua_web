...
--
-- ATTRIBUTION REQUIRED - DO NOT REMOVE. The CC-BY-NC-SA 4.0 license this game
-- ships under requires that credit to the original author (Krystman / Lazy Devs
-- Academy) stays with the work. Keep these comment lines in any copy, fork, or
-- derivative. Removing them violates the license.
--
...
-- The parallax star field lives in the SDK (gt.parallax_*): moving and
...
local parts = pool(56, "age,size2,maxage,blue,spark,dc")
...
function addpart(px,py,vx,vy,age,size2,maxage,blue,spark)
 add(parts,{x=px*16,y=py*16,sx=vx,sy=vy,age=age,size2=size2,maxage=maxage,blue=blue,spark=spark,dc=0})
end
...
function pump_explosions()
 ...
 -- The bulk pool_sprs draw makes the whole swarm one asm blit, so the
 -- alive ceiling goes back up. It's still bounded by the 56-slot pool.
 if #parts >= 52 then
...
-- FIDELITY TRADE-OFF: particle counts cut hard (orig explode 30/20,
-- bigexplode 60/100) - the swarm was slowing the game down considerably.
-- Flash blob + shockwave (the "hit" read) unchanged. Revert toward the
...
function explode(expx,expy,isblue)
 ...
 add(pend,{x=expx,y=expy,blue=isblue,big=0,fleft=24,sleft=14})
end

function bigexplode(expx,expy)
 addpart(expx,expy,0,0,0,50,0,0,0)
 big_shwave(expx,expy)
 add(pend,{x=expx,y=expy,blue=0,big=1,fleft=44,sleft=60})
end
...
-- Fast round-ish disc in THREE rectfills (a plus + inset corner square = an
-- octagon), instead of circfill's ~2r blitter entries. Size-independent cost.
-- inset ~= r*0.29 gives an octagon close to a circle; clamped so tiny discs
-- (r=4,5) still read round. Used only for r>=4 (r<=3 are baked sprites).
...
function bake_particle_discs()
 ...
 -- faithful GameTank bytes for the original page_red / page_blue ramps
 -- (the port stored raw p8 indices here - runtime-array values are NOT
 -- remapped, so 10/8/2 came out the wrong colors). red: white, yellow,
 -- orange, red, dark-red, grey; blue: white, grey, sky, indigo, navy.
 rampc[1]=7 rampc[2]=31 rampc[3]=62 rampc[4]=91 rampc[5]=90 rampc[6]=3
 rampc[7]=6 rampc[8]=190 rampc[9]=140 rampc[10]=169
 local n=1
 for ci=1,10 do
  for r=0,3 do
...
local p8sfx = hexdata("")
...
-- enemy-draw descriptor for gt.pool_edraw (see gt_poolmv.s): per type,
-- [f=1 cell, flash-variant cell, mode(0 skip/1 8x8/2 16x16)]
local edesc = hexdata("15f00194f401b8f601d0310200000000000000000000000000000000")
      ...
      if bj == bi then
       ...
       if e.hp <= 0 then
        ...
        if e.type == 5 then
         ...
         -- capture BEFORE del: del() reuses the slot's x storage for the
         -- pool free-chain, so e.x afterwards is a slot index (pickups all
         -- spawned pinned to a screen edge)
         local emis = e.mission
         del(enemies, e)
         boom_sfx()
         explode(exp + 4, eyp + 4, 0)
         local cherchance = 13
         local scoremult = 1
         if emis == MI_ATTAC then
          scoremult = 2
...
function update_start()
 gt.parallax_move(0)  -- title drift
...
function draw_game()
 ...
 -- drawing enemies: cell derivation + flash + shake + clip + staging all
 -- in one asm walk (gt.pool_edraw); only the boss (type 5, mode 0 in the
 -- descriptor) keeps the Lua path, and only the last wave has one
 local nudge=0
 if (tick%4<2) nudge=1
 gt.pool_edraw(enemies, "aniframe", "type", "flash", "shake", edesc, nudge)
 if wave==lastwave then
  for e in all(enemies) do
   if e.type==5 then
    ...
    local sn=84+bossoff[f]
    if e.flash>0 then
     e.flash-=1
     sn=80   -- boss flash frame from the sheet
    end
    spr(sn,ex3,e.y\16,4,3)
   end
  end
 end
 ...
 -- Particle draw is now ONE bulk asm blit (gt.pool_sprs) instead of a
 -- spr()/circfill() per particle. This pass only picks each particle's baked
 -- disc CELL (age -> ramp tier, size -> radius 0..3) into its byte field, plus
 -- ages/expires it and stamps the white sparks (few, cheap). The swarm then
 -- draws in one asm walk, so the full original particle counts are affordable
 -- again - no more multi-kill vsync spike. (The old huge-radius circfill blobs
 -- are gone: radius clamps to the baked 0..3 discs, which read the same at
 -- explosion speed and cost one blit each.)
 for p2 in all(parts) do
  if p2.spark==1 then
   pset(p2.x\16,p2.y\16,7)     -- sparks: always white, drawn inline
   p2.dc=0                     -- excluded from the bulk pass
  else
   local ci=1
   local a2=p2.age
   if p2.blue==1 then
 ...
 -- ONE asm blit for every disc particle (cell 0 = spark/dead, skipped).
 -- centered like the old spr(...-3,-3): pool_sprs subtracts ox,oy.
 gt.pool_sprs(parts, "dc", 3, 3)
 -- bulk move + damp AFTER the draw (original: draw old pos, then integrate)
