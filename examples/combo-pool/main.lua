...
-- ships under requires that credit to the original author (NuSan) stays with
-- the work. Keep these comment lines in any copy, fork, or derivative.
-- Removing them violates the license.
--
...
local partcol = array8(3)   -- merge-pop colors (cart: p8 8/9/10 red/orange/yellow)
...
local p8sfx = hexdata("")
...
--   scattered singles: motion-trail stamps (trailspr[], rows 12-13 gaps)
-- The sheet additionally holds border/lattice/panel STRIPS so the static
-- art is a handful of wide blits. The field strips sit in tile rows 0-3
-- (bottom border / top border / lattice A / lattice B) because gt.bg_coln
-- composes from the sheet's top 8 KB in ROM (cells 0-127 only); the HUD
-- panel keeps rows 4-5 and the title art lives in rows 12-15 (spr()-only,
-- served from GRAM).
-- ---------------------------------------------------------------------
...
function bake_sprites()
  ...
  while c <= 7 do
    ...
    -- NOTE: colors passed through these helper fns reach sset() as RUNTIME
    -- values = raw GameTank bytes (no PICO-8 remap), so the cart's p8 colors
    -- are written here pre-mapped: shadow p8 1 -> 169, blink backdrop
    -- p8 8 (red) -> 91, aim ring p8 13 (indigo) -> 140.
    bake_circfill(sx, 64 + 9, 5, 169)
    bake_ball_at(sx, 64 + 7, c, gt.rgb(1))
    -- blinking ball (red backdrop): cells 160+
    bake_circfill(sx, 80 + 9, 5, 169)
    bake_ball_at(sx, 80 + 7, c, 91)
    -- launcher ball with the aim ring, no shadow: cells 96+
    bake_circ(sx, 48 + 7, 6, 140)
...
-- gt.phys engine plumbing: bounce flags + contact-pair list + draw cells
...
-- The strips live in sheet tile rows 0-3 (cells 0-63): gt.bg_coln composes
-- from the sheet's TOP 8 KB in ROM (cells 0-127 only; rows 8-15 exist only in
-- GRAM), so compose tiles MUST sit in rows 0-7. Rows 0-3 hold bottom border /
-- top border / lattice A / lattice B; the title art moved to rows 12-15.
-- Cell 0 means "clear" to bg_coln, so the bottom border's column-0 cell is
-- duplicated at cell 64 (row 4 col 0) and composed from there.
function compose_field()
 local i = 0
 while i < 16 do
  fieldcol[1] = 16 + i           -- top border (row 1)
  local r = 2
  while r <= 13 do               -- 12 woven lattice rows (rows 2/3 pairs)
   fieldcol[r] = 32 + i
   fieldcol[r + 1] = 48 + i
   r += 2
  end
  fieldcol[14] = 32 + i          -- the half-pair row at y=104
  fieldcol[15] = i               -- bottom border (row 0)
  if (i == 0) fieldcol[15] = 64  -- cell 0 = "clear"; col 0 duplicated at 64
  fieldcol[16] = 16 + i          -- y>=120 sits under the opaque HUD band
...
function update_game()
  ...
  local s = 0
  while s < 2 do
    gt.phys_step(ballx, bally, ballvx, ballvy, ballc, ballfl, bpairs, 28)
    local k = 1
    while bpairs[k] > 0 do
  ...
  gt.phys_drag(ballvx, ballvy, ballc, 28)
  -- life-cost sum + combo-cooldown decay (step 5), one asm walk (gt.pool_decay)
  lifecost10 = gt.pool_decay(ballc, balllm, ballcost10, 28, 5)
...
function draw_mainmenu()
  ...
  -- here 4 composed row-strip blits - lattice strips live in sheet rows 2-3)
  spr(48, 0, 17, 16, 1)
  spr(32, 0, 25, 16, 2)
  spr(32, 0, 41, 16, 2)
  spr(32, 0, 57, 16, 2)
  ...
  spr(197, titleb, titley, 4, 2)
  spr(229, title2x, title2y, 4, 2)
...
function draw_intromenu()
  ...
  if menuselect > 0 then
    ...
    draw_dbar(29, my + 2, 60, 60, 91, 90, 0)
    spr(242, 60, my + 1)
    spr(204, 70, my, 4, 2)
    draw_dbar(71, my + 2, 15, 15, 91, 90, 7)
  end
...
function draw_game()
  ...
  gt.phys_draw(ballx, bally, bcell, 28)
  ...
  if finish == 0 then
    ...
    if newmaxappear > 0 then
      ...
      spr(236, 2 - ((sl * sl \ 4) * 33) \ 900, 103, 4, 2)
    end
    ...
    if suddendeath == 0 then
      ...
      draw_dbar(50, 124, flr(max(0, pstam)), flr(lstam), 140, 3, 169)
      if maxallowed10 > 0 then
        local warn = 0
        if (lifecost10 + 40 > maxallowed10) warn = 1
        if (warn == 1) spr(204, 49, 0, 4, 2)
        local lifebg = 0
        if (warn == 1) lifebg = 7
        draw_dbar(50, 2, flr(max(0, plife)), flr(llife), 91, 90, lifebg)
      end
    end
  end
  ...
  for p in all(parts) do
    if p.c == 3 then
      circfill(p.x, p.y, 10 - p.t * 20, partcol[p.spread + 1])
    elseif p.c == 2 then
...
function _init()
  -- the field the balls live in: x 4..124, floor at y=112 (the HUD band),
  -- and a falling ball slower than 0.1 px/frame settles instead of bouncing
  gt.phys_bounds(4, 4, 124, 112, 0.1)
  ...
  sd_b[15] = 32
  sd_b[16] = 0
  -- ball palettes: the ORIGINAL cart's tier progression, faithfully. The PICO-8
  -- source uses bpal={1,13,6,9,10,8,14} / bpal2={13,6,7,15,7,14,15} /
  -- bpal3={0,1,13,4,9,2,2} - i.e. tiers go dark-navy, lavender-grey, light-grey,
  -- orange, yellow, red, pink. Players read the merge ladder by THAT color
  -- language, so each PICO-8 index maps to its nearest GameTank byte (the
  -- compiler's own P8->GT table). These arrays are read at runtime, so they hold
  -- raw GameTank bytes; the p8 index is noted per line.
  bpal[1]  = 169   -- p8 1  dark blue    rgb(31,51,74)
  bpal[2]  = 140   -- p8 13 lavender     rgb(117,113,155)
  bpal[3]  = 6     -- p8 6  light grey   rgb(161,161,161)
  bpal[4]  = 62    -- p8 9  orange       rgb(214,155,75)
  bpal[5]  = 31    -- p8 10 yellow       rgb(185,197,65)
  bpal[6]  = 91    -- p8 8  red          rgb(166,74,94)
  bpal[7]  = 94    -- p8 14 pink         rgb(234,140,162)
  bpal2[1] = 140   -- p8 13
  bpal2[2] = 6     -- p8 6
  bpal2[3] = 7     -- p8 7  white
  bpal2[4] = 47    -- p8 15 peach
  bpal2[5] = 7     -- p8 7
  bpal2[6] = 94    -- p8 14
  bpal2[7] = 47    -- p8 15
  bpal3[1] = 0     -- p8 0  black (code substitutes gt.rgb(1) for 0)
  bpal3[2] = 169   -- p8 1
  bpal3[3] = 140   -- p8 13
  bpal3[4] = 51    -- p8 4  brown
  bpal3[5] = 62    -- p8 9
  bpal3[6] = 90    -- p8 2  dark purple
  bpal3[7] = 90    -- p8 2

  partcol[1] = 91  -- p8 8  red
  partcol[2] = 62  -- p8 9  orange
  partcol[3] = 31  -- p8 10 yellow
  ...
  -- free single cells for the 7 trail stamps (rows 12-13 gaps,
  -- next to the relocated title art)
  trailspr[1] = 195
  trailspr[2] = 196
  trailspr[3] = 201
  trailspr[4] = 202
