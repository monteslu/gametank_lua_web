...
const SIGNATURES = {
  ...
  "gt.pool_anim": ["(pool, frameField, spdField, maxField)", "advance animation frames for the whole pool"],
  ...
  "gt.balls_bounds": ["(x0, y0, x1, y1, bounce_min)", "the walls balls bounce in (default: whole screen); bounce_min = fall speed needed to bounce off the floor (0 = always, higher = balls settle)"],
  "gt.balls_step": ["(x, y, vx, vy, act, flags, pairs, n)", "integrate + wall-bounce (walls from gt.balls_bounds) + collision pairs"],
  "gt.balls_drag": ["(vx, vy, act, n)", "apply drag to the ball table"],
  "gt.balls_draw": ["(x, y, cells, n)", "draw the ball table (16x16 blits in bulk asm)"],
  "gt.parts_step": ["(pool)", "step a particle pool (move + age) in asm"],
  "gt.trail_stamp": ["(act, x, y, tx, ty, sprs, n, upd)", "stamp motion trails behind movers"],
  "gt.starfield_init": ["(n)", "parallax starfield: allocate n stars"],
  ...
  "gt.dbar": ["(px, py, v, m, col, col2, bg)", "segmented HUD bar: value v of max m"],
