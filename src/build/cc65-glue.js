 ...
 */
// SYNCHRONOUS: takes a PRE-COMPILED WebAssembly.Module (compiled once, async, at
// worker startup) and runs the tool with `new WebAssembly.Instance` (which is
// synchronous). This is what lets env.runTool be sync - which the SDK's build()
// requires (its run()/cc()/as() calls are synchronous). Warm module in, cart
// bytes out, no await anywhere in the hot path.
export function runWasmTool(wasmModule, { fs, argv, print, printErr }) {
  let exitCode = 0;
  let exited = false;
  let instance;
